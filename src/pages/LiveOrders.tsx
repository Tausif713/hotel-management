...
export default function LiveOrders() {
  ...
  const handleServe = async (dbId: string) => {
    const { error } = await supabase.from('app_orders').update({ status: 'served' }).eq('id', dbId);
    if (error) alert("Error serving order: " + error.message);
  };
              ...
              {filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-slate-50 transition-colors group cursor-pointer">
                  ...
                  <td className="px-8 py-6 text-right">
                    {order.status === 'ready' ? (
                      <button 
                        onClick={() => handleServe(order.dbId)}
